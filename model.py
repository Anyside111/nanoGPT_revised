...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.c_attn = nn.Linear(config.n_embd, total_kv_dim, bias=config.bias)
        ...
        assert self.window_size is None or self.window_size <= config.block_size
        self.window_size = full_size if self.window_size is None else self.window_size
        mask = (torch.tril(torch.ones(full_size, full_size)) -
                torch.tril(torch.ones(full_size, full_size), diagonal=-self.window_size))
        mask[:, :config.n_regist] = 1 # Register tokens can be seen by all tokens
        mask = mask.bool()

        # Register the attention mask
        self.register_buffer("bias", mask.view(1, 1, full_size, full_size))

    def forward(self, x):
        B, T, C = x.size() # batch size, sequence length, embedding dimensionality (n_embd)

        # calculate query, key, values for all heads in batch and split them properly
        qkv = self.c_attn(x)  # combined projection
        q, k, v = qkv.split([self.key_query_dim * self.n_head,
                             self.key_query_dim * self.n_head,
                             self.n_embd], dim=2)

        # Reshape and transpose for multi-head attention
        # From (B, T, key_query_dim * n_head) to (B, n_head, T, key_query_dim)
        k = k.contiguous().view(B, T, self.n_head, self.key_query_dim).transpose(1, 2)
        q = q.contiguous().view(B, T, self.n_head, self.key_query_dim).transpose(1, 2)
        v = v.contiguous().view(B, T, self.n_head, self.n_embd // self.n_head).transpose(1, 2)
        ...
        if self.flash:
            ...
            y = torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=self.bias[:, :, :T, :T],
                                                                 ...
                                                                 is_causal=False) # attn_mask is windowed
        else:
